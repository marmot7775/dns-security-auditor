...
def get_prioritized_selectors(spf_record: str, base_selectors: List[str]) -> List[str]:
    ...
    seen = set()
    priority_selectors = [
        x for x in priority_selectors if not (x in seen or seen.add(x))
    ]
...
def generate_vendor_intelligence_report(spf_record: str) -> str:
    ...
    for cat, cat_vendors in categories.items():
        ...
        for vendor in cat_vendors:
            ...
            report += (
                f"    Testing selectors: {', '.join(vendor['dkim_selectors'])}\n\n"
            )
...
def smart_dkim_check(domain: str, spf_record: Optional[str] = None) -> Dict:
    ...
    for selector in selectors_to_test:
        ...
        try:
            ...
            if not (
                "p=" in dkim_record and not dkim_record.strip().startswith("v=spf1")
            ):
                continue
...
if __name__ == "__main__":
    ...
    print(
        f"HIGH PRIORITY (from SPF): {len([s for s in prioritized if s in ['google', 'k1', 'k2', 'k3', 'em', 's1', 's2']])}"
    )
    print(
        f"LOW PRIORITY (generic): {len(prioritized) - len([s for s in prioritized if s in ['google', 'k1', 'k2', 'k3', 'em', 's1', 's2']])}"
    )
